...
由于写 `Kotlin` 时语法基本上照抄 `Dart`，所以 `isEmpty` 和 `isNotEmpty` 是通过 getter 获取的，然后又写了常规的函数获取
:::
...
<CodeBlock language='kotlin'>
	{`
val isEmpty = fvv.isEmpty && fvv.isEmpty() && !fvv.isNotEmpty && !fvv.isNotEmpty() // 判断是否为空
...
val str = fvv.asString() // 获取字符串
val strs = fvv.asStrings() // 获取字符串组
// 其他依此类推，不再示范
`.trim()}
</CodeBlock>
