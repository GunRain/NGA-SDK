...
:::tip
ShiroSU 模块构建工具 会**默认添加** 实用代码，建议通过它来构建模块
:::
...
:::note
大部分人写的按键监听都有问题，没有考虑到按键事件的 `EV_KEY` 有 `DOWN` 和 `UP` 两个状态
（即 **按下** 和 **松开**）
...
:::tip
此功能为 `run_install_list` 的辅助功能，并非为直接打印所写，故使用 `echo` 而非 `pure_print`
:::
...
若当前架构的共享库文件不存在，会获取 `lib` 目录内第一个架构的，
若还是不存在，则不会继续查找
:::
...
`is_ssu`/`is_shirosu`: 判断是否是 [ShiroSU](https://ssu.oom-wg.dev/)
...
:::warning
通常不建议编写 `boot-completed.sh` 而是在 `service.sh` 中使用 `until_boot`，
除非完全不考虑适配 Magisk
:::
...
:::warning
AW 加密 本身基于 `ksh` 语法编写，不兼容 `POSIX` 语法，切勿使用 `dash` 等解释器执行！
:::

:::tip
如果对 Shell 的安全防护感兴趣，可见[此篇文章](https://oom-wg.dev/posts/shell-obf-enc)
:::
