...
<TypeTable
	type={{
		packageName: {
			...
			description: '生成代码的包名',
			type: 'String',
			default: 'dev.oom_wg.purejoy.fyl.fytxt'
		},
		objectName: {
			required: false,
...
<Callout type='warn'>正则表达式对应的语言标签必须是全大写，所以建议所有语言标签都使用全大写</Callout>
...
<Callout>
	由于要兼顾常规方式与 Compose 方式两个场景，以 **`getter` 调用**为**常规方式**，**函数调用** 为 **Compose
	方式**
</Callout>
