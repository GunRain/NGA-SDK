...
FYTxt 基于 [`FVV`](/docs/fw/) 而不是传统的 xml 存放文本，
...
:::tip
FYTxt 可在 KMP(`org.jetbrains.kotlin.multiplatform`) 或
Android(`com.android.application`/`com.android.library`) 中使用
:::
...
- `packageName: String`(_可选_): 生成代码的包名，默认是 FYTxt 自己的包名
- `objectName: String`(_可选_): 生成语言的根类名，默认是 FYTxt 自己的类名
- `langSrcs: Map<String, Directory>`: 语言组与存放 FVV 文本的目录
- `langAliases: Map<String, String>`(_可选_): 语言与其对应的正则表达式
- `defaultLang: String`: 默认语言
- `composeGen: Boolean`: 为 _Compose MultiPlatform_/_Jetpack Compose_ 生成代码
- `internalClass: Boolean`(_默认开启_): 生成的代码使用 `internal` 修饰符
...
不过当 Gradle 插件在搜索时，会自动将文件夹名称转为全大写，您依旧可以将文件夹名称命名为 `zh_CN` 等内容，
但**不可使用连字符连接**

默认语言也会在运行时自动转为全大写，它将作为 `KDoc` 生成时的默认语言与所有语言的回退文本使用

:::note
如果将默认语言设为 _中文_，且软件仅支持 _中文_、_英文_，那么当系统语言为其他语言时，将显示 **中文**
:::
...
(若以 `common` 目录作为语言组 _Common_，且其他可选选项均为默认配置):
...
:::note
由于要兼顾常规方式与 Compose 方式两个场景，
以 **`getter` 调用**为**常规方式**，**函数调用** 为 **Compose 方式**
:::
