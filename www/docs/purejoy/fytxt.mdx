...
FYTxt 基于 `FVV` 而不是传统的 xml 存放文本，原理是通过 Gradle 插件自动生成 Kotlin 文件，
...
```kotlin
langAliases = mapOf(
    "ZH_CN" to "^ZH_.*(HANS|CN|SG)",
    "ZH" to "^ZH_(?!.*(HANS|CN|SG)).*"
)
```

> 此处代码中 `ZH_CN` 对应 _简体中文_，`ZH` 对应 _繁体中文_
...
_Android JVM_ 输出后的文本默认经过 **Pangu Text** 处理
:::

当 _Android JVM_ 使用 Compose 方式时，语言列表会自动更新，但常规方式需手动调用 `FYTxtConfig.updateTags()` 以更新
